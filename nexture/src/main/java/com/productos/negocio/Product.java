...
public String consultarTodo() {
	...
	String tabla="<table  class = table border=2><th>ID</th><th>Producto</th><th>Cantidad</th><th>Precio</th>";
	ResultSet rs = null;
	...
	public String reporteProducto() {
		...
		try {
			while(rs.next())
			{
				...
				+ "<img src='icons/edit.png' alt='Actualizar' width='25' /></a></td>"
                + "<td><a href='eliminar.jsp?id=" + rs.getInt(1) + "'>"
                + "<img src='icons/delete.png' alt='Eliminar' width='25' /></a></td></tr>";
			}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	//Funcion para revisar imprime null 
	public String reporte() {
		String sql = "SELECT pr.id_pr, pr.nombre_pr, cat.descripcion_cat, pr.cantidad_pr, pr.precio_pr"
				+"FROM tb_producto pr, tb_categoria cat WHERE pr.id_pr = 1 and cat.id_cat = 1;";
		ResultSet rs = null;
		Conexion con=new Conexion();
		rs= con.Consulta(sql);
		String tabla = "<table class='table'> <thead><tr>"
	...
	public String insertarProducto() 
	{
		...
		String sql = "INSERT INTO tb_producto (nombre_pr, id_cat, )";
    ...
    {
    	String sqlSetVal = "SELECT setval(pg_get_serial_sequence('tb_producto', 'id_pr'), (SELECT MAX(id_pr) FROM tb_producto))";
